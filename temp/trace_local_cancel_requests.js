...
const OUTPUT_PATH = path.join(OUTPUT_DIR, 'local-cancel-request-trace.json');
...
(async () => {
  ...
  try {
    ...
    const result = {
      ...
      requests,
      responses,
      cancelRelatedRequests: requests.filter((entry) => /cancel|subscription|nevari-proxy/i.test(entry.url)),
      cancelRelatedResponses: responses.filter((entry) => /cancel|subscription|nevari-proxy/i.test(entry.url)),
      bodyExcerpt: await page.locator('body').textContent().then((value) => String(value || '').replace(/\s+/g, ' ').trim().slice(0, 1000)).catch(() => ''),
    };
