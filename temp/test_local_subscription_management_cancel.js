...
const OUTPUT_PATH = path.join(OUTPUT_DIR, 'local-subscription-management-cancel.json');
const SCREENSHOT_PATH = path.join(OUTPUT_DIR, 'local-subscription-management-cancel.png');
...
(async () => {
  ...
  try {
    ...
    const ctaText = (await page.locator('.customer-subscription-management-cta').textContent().catch(() => '') || '').trim();
    let response = null;

    if (/manage plan/i.test(ctaText)) {
      ...
      const cancelButton = page.getByRole('button', { name: /^Cancel plan$/i }).first();
      await cancelButton.waitFor({ state: 'visible' });
      await cancelButton.click();
      await page.getByText('Are you sure?').waitFor({ state: 'visible' });
      const responsePromise = page.waitForResponse((entry) => entry.url().includes('/subscriptions/cancel') && entry.request().method() === 'POST', { timeout: 45000 });
      await page.getByRole('button', { name: /^Cancel plan$/i }).last().click();
      response = await responsePromise;
    }

    const responseBody = response ? await readResponsePayload(response) : null;
    const errorText = await page.locator('text=Paystack cancellation details are missing').first().textContent().catch(() => '');
    const successText = await page.locator('text=Subscription cancelled.').first().textContent().catch(() => '');
    ...
    const result = {
      ...
      currentUrl: page.url(),
      ctaText,
      cancelAttempted: Boolean(response),
      cancelHttpStatus: response ? response.status() : null,
      cancelResponseBody: responseBody,
      ui: {
        errorText: String(errorText || '').trim(),
        successText: String(successText || '').trim(),
        bodyExcerpt: String(bodyText || '').replace(/\s+/g, ' ').trim().slice(0, 800),
      },
