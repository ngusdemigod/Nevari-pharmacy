...
const OUTPUT_PATH = path.join(OUTPUT_DIR, 'local-cancel-with-csrf.json');
...
(async () => {
  ...
  try {
    ...
    const result = await page.evaluate(async () => {
      const csrfMatch = String(document.cookie || "").match(/(?:^|;\s*)nevari_csrf=([^;]+)/);
      const csrf = csrfMatch ? decodeURIComponent(csrfMatch[1]) : "";
      const endpoint = new URL('/api/nevari-proxy', window.location.origin);
      endpoint.searchParams.set('baseUrl', 'https://nevarihealth.com');
      endpoint.searchParams.set('path', '/subscriptions/cancel');
      const response = await fetch(endpoint.toString(), {
        method: 'POST',
        headers: {
          Accept: 'application/json',
