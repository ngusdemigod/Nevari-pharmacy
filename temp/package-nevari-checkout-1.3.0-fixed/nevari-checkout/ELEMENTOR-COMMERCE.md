...
The production installer bundle is a flat ZIP: `nevari-checkout.php` and the `includes/`, `assets/`, and `templates/` directories are at the archive root. Do not wrap these files in a second `nevari-checkout/` directory; WordPress may register a temporary nested path and report that the plugin file does not exist.

1. Back up the site and replace the previous `nevari-checkout` plugin folder with the 1.3.0 package.
