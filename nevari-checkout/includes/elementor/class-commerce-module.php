...
foreach (array(
    __DIR__ . '/class-woocommerce-adapter.php',
    __DIR__ . '/class-commerce-renderer.php',
    __DIR__ . '/class-ajax-controller.php',
    __DIR__ . '/class-checkout-service.php',
) as $nevari_commerce_module_dependency) {
    if (!is_readable($nevari_commerce_module_dependency)) {
        return;
    }
    require_once $nevari_commerce_module_dependency;
}
...
final class Commerce_Module {
    ...
    public static function instance($plugin_file = '', $core = null) {
        if (null === self::$instance && $plugin_file && $core && class_exists('WooCommerce') && function_exists('WC')
            && class_exists(__NAMESPACE__ . '\\WooCommerce_Adapter')
            && class_exists(__NAMESPACE__ . '\\Commerce_Renderer')
            && class_exists(__NAMESPACE__ . '\\Ajax_Controller')
            && class_exists(__NAMESPACE__ . '\\Checkout_Service')
        ) {
    ...
    private function elementor_version_supported() {
        return class_exists('\\Elementor\\Widget_Base') && defined('ELEMENTOR_VERSION') && version_compare(ELEMENTOR_VERSION, '3.15.0', '>=');
    }
    ...
    public function register_widgets($widgets_manager) {
        if (!$this->elementor_version_supported()) {
        ...
        require_once __DIR__ . '/widgets/class-button-widget.php';

        foreach (array(new Cart_Widget(), new Checkout_Widget(), new Order_Progress_Widget(), new Button_Widget()) as $widget) {
