...
(async () => {
  ...
  const media = page.locator('.product-editor-media-column').first();
  console.log('host', 'http://localhost:3000/admin/storefront');
  console.log('modalCount', await modal.count());
  if (await modal.count()) {
    console.log('modalBox', JSON.stringify(await modal.boundingBox()));
    console.log('mediaBox', JSON.stringify(await media.boundingBox()));
  }
  await page.screenshot({ path: 'D:/dev/nevari-pharmacy-core/temp/product-editor-modal-localhost-open.png', fullPage: true });
  await browser.close();
})();
