...
export default function NurseRequestAdminPanel({ session }) {
  ...
  useEffect(() => {
    ...
    nursesQuery.mutate();
  }, [requestsQuery.mutate, nursesQuery.mutate]);
  ...
  {selected ? <div className="app-modal-layer app-modal-layer-top is-open"><button className="app-modal-backdrop" type="button" aria-label="Close Nurse Request details" onClick={() => setSelectedId("")} /><section className="modal-frame nurse-assignment-panel nurse-request-details-modal" role="dialog" aria-modal="true" aria-label="Nurse Request details"><div className="modal-head"><div><p className="section-kicker">{selected.reference}</p><h2>Nurse Request details</h2></div><button className="icon-button" type="button" aria-label="Close Nurse Request details" onClick={() => setSelectedId("")}>×</button></div><div className="modal-body">
