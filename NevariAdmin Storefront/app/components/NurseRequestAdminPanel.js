...
export default function NurseRequestAdminPanel({ session }) {
  ...
  const requestsQuery = useSWR(session?.baseUrl ? `/api/admin/care-nurse?${query}` : null, (url) => getJson(url, session));
  const nursesQuery = useSWR(session?.baseUrl ? `/api/admin/nurses?${query}` : null, (url) => getJson(url, session));
  ...
  return <section className="page-view active"><section className="panel table-panel">
    ...
    <div className="table-scroll"><table><thead><tr><th>Reference</th><th>Patient</th><th>Status</th><th>Assignee</th><th>Schedule</th><th>Action</th></tr></thead><tbody>
      {requestsQuery.isLoading ? <tr><td colSpan="6">Loading requests…</td></tr> : requests.length ? requests.map((item) => <tr key={item.id}><td>{item.reference}</td><td>{item.patient?.name}</td><td><span className="status-pill warning">{item.status_label}</span></td><td>{item.assignee?.name || "Unassigned"}</td><td>{item.scheduled_at ? new Date(item.scheduled_at).toLocaleString() : "Not scheduled"}</td><td><button className="pill-button" type="button" onClick={() => { setSelectedId(String(item.id)); setNurseId(String(item.assignee?.id || "")); }}>Manage</button></td></tr>) : <tr><td colSpan="6">No Nurse Requests.</td></tr>}
    </tbody></table></div>
  </section>
  {selected ? <section className="panel nurse-assignment-panel"><div className="panel-header"><div><p className="section-kicker">{selected.reference}</p><h2>Assign and schedule</h2></div><button className="pill-button" type="button" onClick={() => setSelectedId("")}>Close</button></div>
    ...
    <div className="governed-user-actions"><button className="pill-button" type="button" disabled={busy || selected.status !== "submitted"} onClick={() => transition("under_review")}>Start review</button><button className="button-primary" type="button" disabled={busy || !nurseId || selected.status !== "under_review"} onClick={() => transition("nurse_assigned")}>Assign nurse</button><button className="button-primary" type="button" disabled={busy || !nurseId || !scheduledAt || selected.status !== "nurse_assigned"} onClick={() => transition("scheduled")}>Confirm schedule</button><button className="pill-button danger" type="button" disabled={busy || ["completed","cancelled","declined"].includes(selected.status)} onClick={() => transition("cancelled")}>Cancel</button></div>
