...
export default function StaffDirectory({ session, search = "" }) {
  ...
  const selectedRoleOptions = selectedRole === "administrator"
  ...
  async function requestAction(user, name, extra = {}) {
    let reason = String(extra.reason || "").trim();
    if (["ban", "suspend", "decline", "reset-password"].includes(name)) {
      reason = window.prompt(`Reason for ${labels[name].toLowerCase()} ${user.display_name || user.user_email}:`, reason) || "";
      if (!reason) return;
    }
    if (name === "access") {
      const targetLabel = extra.role && extra.role !== user.managed_role
        ? `changing ${user.display_name || user.user_email} to ${roles.find(([value]) => value === extra.role)?.[1] || extra.role}`
        : `changing dashboard permissions for ${user.display_name || user.user_email}`;
      reason = window.prompt(`Reason for ${targetLabel}:`, reason) || "";
      if (!reason) return;
    }
      ...
      <section ref={dialogRef} className="staff-fullscreen-modal detail-flat-modal staff-directory-detail-modal" role="dialog" aria-modal="true" aria-labelledby="staff-modal-title">
        ...
        <div className="staff-modal-body">
          ...
          {isAdministrator ? <section className="staff-access-section">
            <label className="detail-field"><span>Role</span><select value={selected.managed_role === "shop_manager" ? "store_admin" : selected.managed_role} disabled={Boolean(busy)} onChange={(event) => requestAction(selected, "access", { role: event.target.value, permissions: selected.permissions || [] })}>{selectedRoleOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
