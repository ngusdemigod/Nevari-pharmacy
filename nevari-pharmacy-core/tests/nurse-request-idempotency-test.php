...
$source = file_get_contents(dirname(__DIR__) . '/includes/class-nevari-nurse-requests.php');
...
foreach ($expectations as $needle => $description) {
    if (strpos($source, $needle) === false) {
        fwrite(STDERR, "Missing Nurse idempotency guarantee: {$description}.\n");
        exit(1);
    }
}

echo "Nurse request idempotency contract tests passed.\n";
